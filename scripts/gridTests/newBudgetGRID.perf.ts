...
export default () => {
	...
	step('Step 4 - Open BGTNBC > New ', async browser => {
			...
			await browser.visit('https://mirvac.itwocx.com/cxR/cx.aspx?page=docs/DocNew0&j=MGR-ENT-MST-001-UAT&dsid=67200&m=&mdu=&bid=&d=&i=&f=&filelist=&linkdocid=&tnd=&transID=&reportID=&due=&z=45125.4681342708')
			await browser.takeScreenshot()	
	...
	step('Step 5 - Enter Title budget transfer description', async browser => {
		...
		await frame1.type(title, 'Flood Test -' + numberRange(1000,350000).toString())
	...
	const frame1 = browser.page.frames().find((frame ) => frame.name().includes('DocNewNewFrame'))
	//let openContract = '//*[@id="tr.001"]/td[8]'  #tr\.001
	let openContract = '#tr\\.001'
	...
	await frame1.type(inputBudget, numberRange(1000,35000).toString())
